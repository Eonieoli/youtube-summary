...
> 💡 **무료 플랜 한도는?**  
> `gemini-2.5-flash` 기준 분당 10 요청(RPM), 하루 250 요청(RPD)까지 무료입니다. 하루 한 번 수십 건의 요약을 처리하는 정도는 여전히 무료 플랜으로 가능하지만, 이전 버전에 비해 일일 한도가 크게 줄었으므로(1,500건 → 250건) 많은 양을 한 번에 처리할 때는 한도 초과에 유의해야 합니다.
...
| `httpx` | 비동기(async) HTTP 요청 라이브러리. YouTube, Supadata API 호출에 사용 |
| `pytz` | 타임존 처리 라이브러리. APScheduler의 Asia/Seoul 설정에 필요 |
| `google-genai` | Google 공식 Gemini SDK. Gemini API 호출에 사용 |
...
> 💡 **YouTube API에 쇼츠 필터 옵션이 없나요?**  
> 없습니다. 그래서 2단계 필터링 전략을 사용합니다. 먼저 `videos` API로 영상 길이(duration)를 조회해 60초 이하는 쇼츠로 즉시 판정합니다. 쇼츠 최대 길이가 3분으로 늘어났기 때문에, 60초 초과 ~ 3분 이하 구간은 `https://www.youtube.com/shorts/{video_id}` URL에 요청을 보내 리다이렉트 여부로 2차 확인합니다. 200이 반환되면 쇼츠, 3xx 리다이렉트가 반환되면 일반 영상입니다. 3분 초과 영상은 추가 확인 없이 일반 영상으로 처리합니다.

> 💡 **`filter_shorts()`에서 API를 두 번 호출하지 않나요?**  
> `videos` API 호출은 영상 전체를 ID를 쉼표로 이어 한 번에 묶어서 조회합니다. 추가 quota 소모는 채널당 1 unit으로 매우 저렴합니다. 리다이렉트 확인은 60초~3분 구간 영상에만 이루어지므로, 대부분의 경우 추가 HTTP 요청은 0~1회에 그칩니다.
...
> 💡 **왜 `google-genai` SDK를 쓰나요?**  
> 기존에는 `httpx`로 Gemini REST API를 직접 호출했습니다. SDK를 쓰면 HTTP 통신, 인증, 응답 파싱을 라이브러리가 대신 처리해줘서 코드가 훨씬 단순해집니다. 응답 텍스트도 `data["candidates"][0]["content"]["parts"][0]["text"]` 같은 복잡한 경로 대신 `response.text` 한 줄로 꺼낼 수 있습니다.

> 💡 **`client`를 함수 밖에서 초기화하는 이유는?**  
> `summarize()` 함수는 영상 수만큼 반복 호출됩니다. 함수 안에서 `genai.Client()`를 만들면 호출마다 클라이언트 객체가 새로 생성됩니다. 모듈 최상단에서 한 번만 초기화하면 이 비용을 줄일 수 있습니다.

> 💡 **`temperature` 설정은 어떻게 하나요?**  
> `google-genai` SDK에서는 `GenerateContentConfig`를 통해 설정할 수 있습니다. 기본값으로도 요약 품질이 충분하지만, 직접 조정하고 싶다면 아래처럼 추가하면 됩니다.
> ```python
